...
Route::middleware(['auth'])->group(function () {
    ...
    Route::prefix('guru')->name('guru.')->group(function () {
        ...
        Route::get('/penilaian/{penilaian}/export-pdf', [GuruDashboardController::class, 'exportPdf'])
            ->name('penilaian.export-pdf');

        // Jurnal (tetap ada sesuai permintaan sebelumnya)
        Route::patch('/jurnal/{jurnal}/validasi', [GuruDashboardController::class, 'validasiJurnal'])->name('jurnal.validasi');
        Route::get('/jurnal', [GuruDashboardController::class, 'indexJurnal'])->name('jurnal.index');
        Route::post('/jurnal/bulk-validasi', [GuruDashboardController::class, 'bulkValidasiJurnal'])->name('jurnal.bulk-validasi');
    });
});
